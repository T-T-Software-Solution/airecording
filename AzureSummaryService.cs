...
public class AzureSummaryService
{
    ...
    public async Task<(string title, string summary)> SummarizeAsync(string text, string language = "auto")
    {
        ...
        try
        {
            ...
            // Option 1: Simple Clean-up Prompt
            var systemPrompt = language.ToLower() switch
            {
                "th" => "คุณคือผู้ช่วย AI ที่เชี่ยวชาญการแก้ไขและจัดรูปแบบข้อความดิบให้สะอาดและอ่านง่าย กรุณาตอบในรูปแบบ JSON ที่มี \"title\" (string) และ \"detailed_content\" (string ในรูปแบบ Markdown)",
                "en" => "You are an AI assistant specializing in correcting and formatting raw text to be clean and readable. Respond in JSON format with \"title\" (string) and \"detailed_content\" fields.",
                _ => "You are an AI assistant specializing in correcting and formatting raw text to be clean and readable. Respond in the same language as the input. Respond in JSON format with \"title\" (string) and \"detailed_content\" fields."
            };

            var userPrompt = language.ToLower() switch
            {
                "th" => $"กรุณาปรับปรุงข้อความนี้ให้อ่านง่ายขึ้น โดยทำตามขั้นตอนต่อไปนี้:\n1. สร้างหัวข้อสั้นๆ ที่สรุปเนื้อหา\n2. แก้ไขการสะกดคำและไวยากรณ์ให้ถูกต้อง\n3. ปรับปรุงประโยคให้อ่านลื่นขึ้น โดยอาจลบคำฟุ่มเฟือยออก\n4. จัดรูปแบบเนื้อหาใหม่โดยแบ่งเป็นย่อหน้าให้ชัดเจน\n5. ใช้ขีดหน้า (-) สำหรับรายการต่างๆ หากมี\n\nตอบกลับในรูปแบบ JSON เท่านั้น:\n{{\n  \"title\": \"หัวข้อสั้นๆ\",\n  \"detailed_content\": \"ย่อหน้าที่ 1 ที่ผ่านการแก้ไขและจัดรูปแบบแล้ว\\n\\nย่อหน้าที่ 2 ที่ผ่านการแก้ไขแล้ว\"\n}}\n\nข้อความ:\n{text}",
                "en" => $"Please improve this text to make it easy to read by following these steps:\n1. Create a short, descriptive title.\n2. Correct spelling and grammar mistakes.\n3. Rephrase sentences for better flow and remove filler words.\n4. Format the content into clear paragraphs.\n5. Use bullet points (-) for any lists.\n\nRespond in JSON format only:\n{{\n  \"title\": \"Short Descriptive Title\",\n  \"detailed_content\": \"The first paragraph, corrected and formatted.\\n\\nThe second paragraph, corrected and formatted.\"\n}}\n\nText:\n{text}",
                _ => $"Please improve this text to make it easy to read by following these steps:\n1. Create a short, descriptive title.\n2. Correct spelling and grammar mistakes.\n3. Rephrase sentences for better flow and remove filler words.\n4. Format the content into clear paragraphs.\n5. Use bullet points (-) for any lists.\n\nRespond in JSON format only:\n{{\n  \"title\": \"Short Descriptive Title\",\n  \"detailed_content\": \"The first paragraph, corrected and formatted.\\n\\nThe second paragraph, corrected and formatted.\"\n}}\n\nText:\n{text}"
            };
